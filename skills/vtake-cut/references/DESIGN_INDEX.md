...
This directory is a **reference library** for the vtake skill. Style, layout,
...
self-contained HTML fragment that follows the vtake card-HTML contract
...
3. **All `<style>` rules must be prefixed with `.card[data-card-id="..."]`** —
   the vtake sanitizer auto-scopes them, but write them already-scoped to
   stay readable
